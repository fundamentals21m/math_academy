...
import type { RaceGameResult } from '@shared/gamification';
import {
  generateProblemSet,
  validateAnswer,
  type BaseConversionProblem,
  type BaseRacingTier,
...
export interface GameSession {
  ...
  roundCount: number;
  currentRound: number;
  currentProblem: BaseConversionProblem | null;
  problems: BaseConversionProblem[];
...
export interface GameSessionActions {
  startGame: (tier: BaseRacingTier, roundCount: number) => void;
...
export function useGameSession(): [GameSession, GameSessionActions] {
   ...
   */
  const startGame = useCallback((tier: BaseRacingTier, roundCount: number) => {
   ...
   */
  const submitAnswer = useCallback((answer: string, responseTimeMs: number) => {
    setSession((prev) => {
