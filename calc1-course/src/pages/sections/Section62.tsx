  ...
  return (
    <LessonLayout sectionId={62}>
      ...
      <Callout type="theorem" title="Euler's Formula">
        <p>For any real number <InlineMath>{`\\theta`}</InlineMath>:</p>
      ...
      <Callout type="info" title="Motivation">
        ...
        <p>
          The real part is the Taylor series for <InlineMath>{`\\cos\\theta`}</InlineMath>,
          and the imaginary part is the series for <InlineMath>{`\\sin\\theta`}</InlineMath>.
        </p>
      </Callout>

      <Callout type="theorem" title="Euler's Identity">
        <p>Setting <InlineMath>{`\\theta = \\pi`}</InlineMath> in Euler's formula:</p>
        <MathBlock>{`e^{i\\pi} + 1 = 0`}</MathBlock>
        <p>
          This remarkable equation relates five fundamental constants:
          <InlineMath>{`e`}</InlineMath>, <InlineMath>{`i`}</InlineMath>, <InlineMath>{`\\pi`}</InlineMath>,
          <InlineMath>{`1`}</InlineMath>, and <InlineMath>{`0`}</InlineMath>.
        </p>
      </Callout>
      ...
      <Callout type="definition" title="Exponential Form">
        <p>
          Using Euler's formula, the polar form <InlineMath>{`z = r(\\cos\\theta + i\\sin\\theta)`}</InlineMath>
          can be written as:
        </p>
        <MathBlock>{`z = re^{i\\theta}`}</MathBlock>
        <p>
          where <InlineMath>{`r = |z|`}</InlineMath> and <InlineMath>{`\\theta = \\arg(z)`}</InlineMath>.
        </p>
      </Callout>
      ...
      <Callout type="example" title="Example 1: Converting to Exponential Form">
        ...
        <ul className="list-disc pl-6 space-y-2">
          <li>
            <InlineMath>{`z = 1 + i`}</InlineMath>: <InlineMath>{`r = \\sqrt{2}`}</InlineMath>,
            <InlineMath>{`\\theta = \\pi/4`}</InlineMath>, so <InlineMath>{`z = \\sqrt{2}e^{i\\pi/4}`}</InlineMath>
          </li>
          <li>
            <InlineMath>{`z = -2`}</InlineMath>: <InlineMath>{`r = 2`}</InlineMath>,
            <InlineMath>{`\\theta = \\pi`}</InlineMath>, so <InlineMath>{`z = 2e^{i\\pi}`}</InlineMath>
          </li>
          <li>
            <InlineMath>{`z = 3i`}</InlineMath>: <InlineMath>{`r = 3`}</InlineMath>,
            <InlineMath>{`\\theta = \\pi/2`}</InlineMath>, so <InlineMath>{`z = 3e^{i\\pi/2}`}</InlineMath>
          </li>
          <li>
            <InlineMath>{`z = 1 - \\sqrt{3}i`}</InlineMath>: <InlineMath>{`r = 2`}</InlineMath>,
            <InlineMath>{`\\theta = -\\pi/3`}</InlineMath>, so <InlineMath>{`z = 2e^{-i\\pi/3}`}</InlineMath>
          </li>
        </ul>
      </Callout>
      ...
      <Callout type="example" title="Example 2: Multiplication">
        <p>
          Compute <InlineMath>{`(2e^{i\\pi/6})(3e^{i\\pi/4})`}</InlineMath>.
        </p>
      ...
      <Callout type="theorem" title="Conjugate in Exponential Form">
        <p>
          If <InlineMath>{`z = re^{i\\theta}`}</InlineMath>, then:
        </p>
      ...
      <Callout type="theorem" title="De Moivre's Theorem">
        <p>For any integer <InlineMath>{`n`}</InlineMath>:</p>
      ...
      <Callout type="example" title="Example 3: Computing Powers">
        <p>Compute <InlineMath>{`(1 + i)^8`}</InlineMath>.</p>
        <p><strong>Solution:</strong></p>
      ...
      <Callout type="example" title="Example 4: Another Power">
        <p>Compute <InlineMath>{`(\\sqrt{3} + i)^6`}</InlineMath>.</p>
        <p><strong>Solution:</strong></p>
      ...
      <Callout type="theorem" title="Deriving Trig Identities">
        <p>
          ...
          Expand <InlineMath>{`(\\cos\\theta + i\\sin\\theta)^n`}</InlineMath> and compare real and
          imaginary parts.
        </p>
      </Callout>

      <Callout type="example" title="Example 5: Double Angle Formulas">
        <p>Derive the double angle formulas using <InlineMath>{`n = 2`}</InlineMath>.</p>
        <p><strong>Solution:</strong></p>
        <p>By De Moivre:</p>
      ...
      <Callout type="example" title="Example 6: Triple Angle Formula">
        <p>Find <InlineMath>{`\\cos 3\\theta`}</InlineMath> and <InlineMath>{`\\sin 3\\theta`}</InlineMath>.</p>
        <p><strong>Solution:</strong></p>
      ...
      <Callout type="theorem" title="nth Roots">
        <p>
          Every nonzero complex number <InlineMath>{`z`}</InlineMath> has exactly <InlineMath>{`n`}</InlineMath>
          distinct <InlineMath>{`n`}</InlineMath>th roots. If <InlineMath>{`z = re^{i\\theta}`}</InlineMath>, the
          <InlineMath>{`n`}</InlineMath>th roots are:
        </p>
      ...
      <Callout type="info" title="Geometric Interpretation">
        <p>
          The <InlineMath>{`n`}</InlineMath>th roots of <InlineMath>{`z`}</InlineMath> are equally spaced
          around a circle of radius <InlineMath>{`r^{1/n}`}</InlineMath>, with angular separation
          <InlineMath>{`2\\pi/n`}</InlineMath>.
        </p>
      </Callout>
      ...
      <Callout type="example" title="Example 7: Cube Roots of 1">
        ...
        <p><strong>Solution:</strong></p>
        <p>
          Write <InlineMath>{`1 = e^{i \\cdot 0}`}</InlineMath>. The cube roots are:
        </p>
        <MathBlock>{`w_k = 1^{1/3} e^{i(0 + 2\\pi k)/3} = e^{2\\pi i k/3}, \\quad k = 0, 1, 2`}</MathBlock>
        <ul className="list-disc pl-6 space-y-1">
          <li><InlineMath>{`w_0 = e^0 = 1`}</InlineMath></li>
          <li><InlineMath>{`w_1 = e^{2\\pi i/3} = \\cos\\frac{2\\pi}{3} + i\\sin\\frac{2\\pi}{3} = -\\frac{1}{2} + \\frac{\\sqrt{3}}{2}i`}</InlineMath></li>
          <li><InlineMath>{`w_2 = e^{4\\pi i/3} = \\cos\\frac{4\\pi}{3} + i\\sin\\frac{4\\pi}{3} = -\\frac{1}{2} - \\frac{\\sqrt{3}}{2}i`}</InlineMath></li>
        </ul>
        <p className="mt-2">
      ...
      <Callout type="example" title="Example 8: Fourth Roots of -16">
        <p>Find all fourth roots of <InlineMath>{`-16`}</InlineMath>.</p>
        <p><strong>Solution:</strong></p>
        <p>Write <InlineMath>{`-16 = 16e^{i\\pi}`}</InlineMath>. The fourth roots are:</p>
        <MathBlock>{`w_k = 16^{1/4} e^{i(\\pi + 2\\pi k)/4} = 2e^{i\\pi(1 + 2k)/4}, \\quad k = 0, 1, 2, 3`}</MathBlock>
        <ul className="list-disc pl-6 space-y-1">
          <li><InlineMath>{`w_0 = 2e^{i\\pi/4} = 2\\left(\\frac{\\sqrt{2}}{2} + \\frac{\\sqrt{2}}{2}i\\right) = \\sqrt{2} + \\sqrt{2}i`}</InlineMath></li>
          <li><InlineMath>{`w_1 = 2e^{i3\\pi/4} = -\\sqrt{2} + \\sqrt{2}i`}</InlineMath></li>
          <li><InlineMath>{`w_2 = 2e^{i5\\pi/4} = -\\sqrt{2} - \\sqrt{2}i`}</InlineMath></li>
          <li><InlineMath>{`w_3 = 2e^{i7\\pi/4} = \\sqrt{2} - \\sqrt{2}i`}</InlineMath></li>
        </ul>
      </Callout>

      <Callout type="example" title="Example 9: Square Roots of i">
        <p>Find both square roots of <InlineMath>{`i`}</InlineMath>.</p>
        <p><strong>Solution:</strong></p>
        <p>Write <InlineMath>{`i = e^{i\\pi/2}`}</InlineMath>. The square roots are:</p>
        <MathBlock>{`w_k = e^{i(\\pi/2 + 2\\pi k)/2} = e^{i\\pi(1 + 4k)/4}, \\quad k = 0, 1`}</MathBlock>
        <ul className="list-disc pl-6 space-y-1">
          <li><InlineMath>{`w_0 = e^{i\\pi/4} = \\frac{\\sqrt{2}}{2} + \\frac{\\sqrt{2}}{2}i = \\frac{1 + i}{\\sqrt{2}}`}</InlineMath></li>
          <li><InlineMath>{`w_1 = e^{i5\\pi/4} = -\\frac{\\sqrt{2}}{2} - \\frac{\\sqrt{2}}{2}i = -\\frac{1 + i}{\\sqrt{2}}`}</InlineMath></li>
        </ul>
        <p className="mt-2">
          Verify: <InlineMath>{`\\left(\\frac{1+i}{\\sqrt{2}}\\right)^2 = \\frac{1 + 2i - 1}{2} = i`}</InlineMath> ✓
        </p>
      </Callout>
      ...
      <Callout type="definition" title="nth Roots of Unity">
        <p>
          The <strong><InlineMath>{`n`}</InlineMath>th roots of unity</strong> are the solutions to
          <InlineMath>{`z^n = 1`}</InlineMath>:
        </p>
        <MathBlock>{`\\omega_k = e^{2\\pi i k/n}, \\quad k = 0, 1, \\ldots, n-1`}</MathBlock>
        <p>
          If <InlineMath>{`\\omega = e^{2\\pi i/n}`}</InlineMath> is the <strong>primitive</strong>
          <InlineMath>{`n`}</InlineMath>th root of unity, then all roots are <InlineMath>{`1, \\omega, \\omega^2, \\ldots, \\omega^{n-1}`}</InlineMath>.
        </p>
      </Callout>

      <Callout type="theorem" title="Sum of Roots of Unity">
        <p>The <InlineMath>{`n`}</InlineMath>th roots of unity sum to zero:</p>
      ...
      <Callout type="definition" title="Complex Exponential">
        <p>For any complex number <InlineMath>{`z = x + iy`}</InlineMath>:</p>
      ...
      <Callout type="example" title="Example 10: Computing Complex Exponentials">
        <ul className="list-disc pl-6 space-y-2">
          <li>
            <InlineMath>{`e^{1 + i\\pi/2} = e^1 \\cdot e^{i\\pi/2} = e(\\cos\\frac{\\pi}{2} + i\\sin\\frac{\\pi}{2}) = ei`}</InlineMath>
          </li>
          <li>
            <InlineMath>{`e^{2 - i\\pi} = e^2 \\cdot e^{-i\\pi} = e^2(\\cos\\pi - i\\sin\\pi) = -e^2`}</InlineMath>
          </li>
          <li>
            <InlineMath>{`e^{i\\pi/3} = \\cos\\frac{\\pi}{3} + i\\sin\\frac{\\pi}{3} = \\frac{1}{2} + \\frac{\\sqrt{3}}{2}i`}</InlineMath>
          </li>
        </ul>
      </Callout>
      ...
      <Callout type="theorem" title="Periodicity">
        <p>
          The complex exponential is periodic with period <InlineMath>{`2\\pi i`}</InlineMath>:
        </p>
      ...
      <ul>
        <li>
          <strong>Euler's formula:</strong> <InlineMath>{`e^{i\\theta} = \\cos\\theta + i\\sin\\theta`}</InlineMath>
        </li>
        <li>
          <strong>Exponential form:</strong> <InlineMath>{`z = re^{i\\theta}`}</InlineMath>
        </li>
        <li>
          <strong>De Moivre:</strong> <InlineMath>{`(e^{i\\theta})^n = e^{in\\theta}`}</InlineMath>
        </li>
        <li>
          <strong>nth roots:</strong> <InlineMath>{`w_k = r^{1/n}e^{i(\\theta + 2\\pi k)/n}`}</InlineMath> for <InlineMath>{`k = 0, \\ldots, n-1`}</InlineMath>
        </li>
        <li>
          <strong>Roots of unity:</strong> <InlineMath>{`e^{2\\pi ik/n}`}</InlineMath> form vertices of regular <InlineMath>{`n`}</InlineMath>-gon
        </li>
        <li>
          <strong>Complex exponential:</strong> <InlineMath>{`e^{x+iy} = e^x(\\cos y + i\\sin y)`}</InlineMath>
        </li>
        <li>
          <strong>Periodicity:</strong> <InlineMath>{`e^{z + 2\\pi i} = e^z`}</InlineMath>
        </li>
      </ul>
    </LessonLayout>
  );
