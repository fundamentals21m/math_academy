...
export const syncScores = functions.https.onCall(
  async (
    ...
    const validCourses: CourseId[] = ['ba', 'crypto', 'aa'];
    const batch = admin.firestore().batch();
    const userScores: Record<CourseId, number> = { ba: 0, crypto: 0, aa: 0 };
    ...
    // Calculate total XP
    const totalXP = userScores.ba + userScores.crypto + userScores.aa;
...
export const getLeaderboard = functions.https.onCall(
  async (
    ...
    if (!courseId || !validOptions.includes(courseId)) {
      throw new functions.https.HttpsError(
        'invalid-argument',
        'courseId must be "ba", "crypto", "aa", or "overall"'
      );
    }
