...
function extractScoresFromStorage(): ScoreUpdate[] | null {
  try {
    ...
    const scores: Record<CourseId, number> = { ba: 0, crypto: 0, aa: 0, linalg: 0, advlinalg: 0, islr: 0, ra: 0, calc1: 0, calc_lib_art: 0 };
    ...
    for (const [sectionId, sectionData] of Object.entries(state.sections)) {
      ...
      if (section.quizAttempts) {
        for (const attempt of section.quizAttempts) {
          // Calculate XP from difficulty and score
          if (attempt.difficulty && typeof attempt.score === 'number') {
            const baseXP = {
              easy: XP_CONFIG.QUIZ_EASY,
              medium: XP_CONFIG.QUIZ_MEDIUM,
              hard: XP_CONFIG.QUIZ_HARD,
            }[attempt.difficulty] || XP_CONFIG.QUIZ_EASY;

            let xpEarned = Math.round(baseXP * (attempt.score / 100));

            // Add perfect quiz bonus
            if (attempt.correctAnswers === attempt.totalQuestions && attempt.totalQuestions > 0) {
              xpEarned = Math.round(xpEarned * (1 + XP_CONFIG.QUIZ_PERFECT_BONUS));
            }

            scores[course] += xpEarned;
          }
        }
      }
...
export class SyncManager {
  ...
  getLocalScores(): Record<CourseId, number> | null {
    ...
    return scores.reduce(
      ...
      { ba: 0, crypto: 0, aa: 0, linalg: 0, advlinalg: 0, islr: 0, ra: 0, calc1: 0, calc_lib_art: 0 } as Record<CourseId, number>
    );
  }
}
