  ...
  const handleNext = () => {
    if (currentIndex < totalQuestions - 1) {
      ...
      const lastAnswerCorrect = showResult && (
        ...
        (questionType === 'text' && textAnswer.toLowerCase() === String(currentQuestion.correctAnswer ?? '').toLowerCase())
      );
      const finalScore = answeredCorrectly + (lastAnswerCorrect ? 1 : 0);
      const finalPercentage = globalThis.Math.min(100, globalThis.Math.round((finalScore / totalQuestions) * 100));
    ...
    <div className="mt-12 p-6 rounded-2xl bg-dark-800/50 border border-dark-700/50">
      ...
      {questionType === 'numeric' && (
        <div className="space-y-4 mb-6">
          <input
            type="number"
            step={currentQuestion.numericRange?.precision ? 1 / globalThis.Math.pow(10, currentQuestion.numericRange.precision) : 1}
