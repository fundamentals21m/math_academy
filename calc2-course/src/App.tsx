...
// Map part IDs to lazy components
const problemsPartComponents: Record<number, React.LazyExoticComponent<React.ComponentType>> = {
  1: ProblemsPart01,
  2: ProblemsPart02,
  3: ProblemsPart03,
  4: ProblemsPart04,
...
function ProblemsPartRouter() {
  const { partId } = useParams<{ partId: string }>();
  const id = parseInt(partId || '1', 10);

  const PartComponent = problemsPartComponents[id];
  ...
  return (
    <div className="min-h-screen bg-dark-950 flex items-center justify-center">
      <div className="text-center">
        <h1 className="text-2xl font-bold text-dark-100 mb-4">Problem Set Not Found</h1>
        <p className="text-dark-400">This problem set is not yet available.</p>
      </div>
    </div>
  );
}
        ...
        <Routes>
          ...
          {FEATURES.problemSets && (
            <>
              ...
              />
              <Route path="/problems/:partId" element={<ProblemsPartRouter />} />
            </>
          )}
