...
from config import EPISODE_SECTIONS, VIDEO_WIDTH, VIDEO_HEIGHT, TOTAL_EPISODES
...
def create_chrome_driver(width: int = VIDEO_WIDTH, height: int = VIDEO_HEIGHT):
    """Create and configure Chrome WebDriver with exact viewport dimensions."""
    ...
    # Use CDP to set exact viewport dimensions (window-size sets outer window, not viewport)
...
def main():
    ...
    parser.add_argument("--all", "-a", action="store_true", help=f"Render all episodes 1-{TOTAL_EPISODES}")
