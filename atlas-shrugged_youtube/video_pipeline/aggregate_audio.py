#!/usr/bin/env python3
"""
Aggregate Audio Script for Atlas Shrugged YouTube Series
...
Usage:
    ...
    python aggregate_audio.py --all              # All 30 episodes
...
from config import PROJECT_DIR as BASE_DIR
...
def aggregate_episode_audio(episode_num: int, verbose: bool = True) -> Path:
    ...
    Args:
        episode_num: Episode number (1-30)
...
def main():
    ...
    group.add_argument(
        "--episode", "-e",
        type=int,
        help="Single episode number (1-30)"
    )
    group.add_argument(
        "--all", "-a",
        action="store_true",
        help="Process all 30 episodes"
    )
    group.add_argument(
        "--episodes",
        type=str,
