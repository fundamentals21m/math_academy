...
export function PellEquationSolver({ className = '' }: Props) {
  const [dValue, setDValue] = useState(2);
  ...
  const validD = dValue > 0 && !isSquare(dValue) && dValue <= 100;

  // Find fundamental solution using continued fraction expansion
  const fundamentalSolution = useMemo((): { xVal: bigint; yVal: bigint } | null => {
    ...
    if (pCurr * pCurr - BigInt(dValue) * qCurr * qCurr === BigInt(1)) {
      return { xVal: pCurr, yVal: qCurr };
    }
    ...
    for (let iter = 0; iter < 1000; iter++) {
      cfM = cfD * cfA - cfM;
      cfD = Math.floor((dValue - cfM * cfM) / cfD);
      ...
      if (pNext * pNext - BigInt(dValue) * qNext * qNext === BigInt(1)) {
        return { xVal: pNext, yVal: qNext };
      }
  ...
  const solutions = useMemo((): PellSolution[] => {
    ...
    const { xVal: x1, yVal: y1 } = fundamentalSolution;

    let xCurr = x1;
    let yCurr = y1;

    for (let solIdx = 1; solIdx <= numSolutions; solIdx++) {
      sols.push({ xVal: xCurr, yVal: yCurr, index: solIdx });

      // (x_{n+1}, y_{n+1}) from (x_n + y_n * sqrt(D)) * (x_1 + y_1 * sqrt(D))
      const xNext = xCurr * x1 + yCurr * y1 * BigInt(dValue);
    ...
    return sols;
  }, [fundamentalSolution, numSolutions, dValue]);
    ...
    <div className={`p-6 rounded-2xl bg-dark-800/50 border border-dark-700/50 ${className}`}>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div>
          ...
          <input
            ...
            max="100"
            value={dValue}
            onChange={(evt) => setDValue(parseInt(evt.target.value) || 2)}
        ...
        <div>
          ...
          <input
            ...
            max="10"
            value={numSolutions}
            onChange={(evt) => setNumSolutions(Math.min(10, Math.max(1, parseInt(evt.target.value) || 1)))}
            className="w-full px-4 py-2 rounded-xl bg-dark-700 border border-dark-600 text-dark-100 focus:border-primary-500 focus:outline-none"
          />
        </div>
      </div>
      ...
      <div className="p-4 rounded-xl bg-dark-900/50 border border-dark-700 mb-6 text-center">
        <span className="text-2xl font-mono">
          ...
          <span className="text-amber-400">{dValue}</span>
      ...
      {!validD && (
        <div className="p-4 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-400 mb-6">
          {isSquare(dValue)
            ? `${dValue} is a perfect square. D must be a non-square positive integer.`
            : dValue <= 0
        ...
        <>
          ...
          >
            ...
            <div className="font-mono text-lg">
              <span className="text-primary-400">x₁ = {formatBigInt(fundamentalSolution.xVal)}</span>
              <span className="text-dark-400 mx-3">,</span>
              <span className="text-primary-400">y₁ = {formatBigInt(fundamentalSolution.yVal)}</span>
            </div>
            <p className="text-dark-400 text-sm mt-2">
              Verify: {formatBigInt(fundamentalSolution.xVal)}² - {dValue} × {formatBigInt(fundamentalSolution.yVal)}² = 1
            </p>
          </motion.div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {solutions.map((sol) => {
                  const verification = sol.xVal * sol.xVal - BigInt(dValue) * sol.yVal * sol.yVal;
                  return (
                    <motion.tr
                      key={sol.index}
                      initial={{ opacity: 0, x: -10 }}
                      animate={{ opacity: 1, x: 0 }}
                      transition={{ delay: sol.index * 0.1 }}
                      className="border-b border-dark-700"
                    >
                      <td className="p-3 font-mono text-amber-400">{sol.index}</td>
                      <td className="p-3 font-mono text-dark-200 text-sm">
                        {formatBigInt(sol.xVal)}
                      </td>
                      <td className="p-3 font-mono text-dark-200 text-sm">
                        {formatBigInt(sol.yVal)}
                      </td>
                      <td className="p-3">
