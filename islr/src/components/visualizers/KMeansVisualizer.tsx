...
export function KMeansVisualizer() {
  ...
  const initialPoints: Point[] = useMemo(() => {
    ...
    // Cluster 1 (top-left)
    for (let i = 0; i < 8; i++) {
      pts.push({ x: 50 + Math.random() * 60, y: 40 + Math.random() * 50, cluster: -1 });
    }
    // Cluster 2 (top-right)
    for (let i = 0; i < 8; i++) {
      pts.push({ x: 200 + Math.random() * 60, y: 50 + Math.random() * 50, cluster: -1 });
    }
    // Cluster 3 (bottom-center)
    for (let i = 0; i < 8; i++) {
      pts.push({ x: 120 + Math.random() * 80, y: 160 + Math.random() * 50, cluster: -1 });
    }
    return pts;
  }, []);
  ...
  const reset = () => {
    ...
    setCentroids(Array.from({ length: k }, () => ({
      x: 50 + Math.random() * 220,
      y: 40 + Math.random() * 180,
    })));
    setIteration(0);
