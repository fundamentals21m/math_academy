...
function extractScoresFromStorage(): ScoreUpdate[] | null {
  try {
    ...
    const scores: Record<CourseId, number> = { ba: 0, crypto: 0, aa: 0, linalg: 0, advlinalg: 0, template: 0, islr: 0, ra: 0, calc1: 0, calc_lib_art: 0, west_graphs: 0 };
    ...
    return [
      ...
      { courseId: 'west_graphs', xp: scores.west_graphs },
    ];
  } catch (error) {
...
export class SyncManager {
  ...
  getLocalScores(): Record<CourseId, number> | null {
    ...
    return scores.reduce(
      ...
      { ba: 0, crypto: 0, aa: 0, linalg: 0, advlinalg: 0, template: 0, islr: 0, ra: 0, calc1: 0, calc_lib_art: 0, west_graphs: 0 } as Record<CourseId, number>
    );
  }
}
